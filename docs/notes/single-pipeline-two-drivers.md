...
> Phase 0 (DailySplitter)·Phase 4 (Airflow)를 얹으면서 헷갈리기 쉬운 점을 정리한 참고 문서.
> "기존 vs Airflow 두 개의 파이프라인이 생기는 것 아닌가?", "Airflow가 기존을 대체하나?"에 대한 답.

## 핵심 한 줄

**파이프라인은 하나.**
같은 변환 코어를 (1) 손으로 `backfill`, (2) Airflow로 일별 자동 — **두 방식 (driver)으로 돌리는 것**이며, `DailySplitter`는 (2)에서만 필요한 **입력 랜딩 (Bronze) 추가**.
Silver/Gold는 하나의 같은 테이블이라, Airflow는 "결과를 대체"하는 게 아니라 "그 결과를 만드는 실행을 자동화·통합"함.

## 1. 기존 파이프라인엔 별도 Bronze 단계가 없음 (오해 바로잡기)

기존 파이프라인의 "일별 파티션 분리"는 **Silver를 쓸 때** 일어남 — `Main`이 월 CSV를 통째로 읽어 변환한 뒤, `PartitionWriter`가 `output/activity/event_date=YYYY-MM-DD/`로 날짜별로 기록함 (`Main.scala`의 write 루프, `PartitionWriter.writePartition`)
즉 **일별 파티셔닝 = Silver 출력 단계**이고, 물리적으로 분리된 Bronze 폴더는 존재하지 않았음.

`DailySplitter`가 새로 만드는 것은 **입력 (Bronze) 쪽의 일별 랜딩** (`data/daily/`)이고, 이건 Silver의 일별 파티셔닝을 *대신*하는 게 아니라 그 **앞에 새로 추가**되는 단계.
Silver의 날짜별 기록은 두 경로 모두에서 `Main` 안에서 그대로 일어남.

## 2. 질문별 답

**Q1. 기존 파이프라인 흐름은?** — "월 CSV → 일별 분리 → Bronze"라는 중간 단계는 **없음**. 정확히는:
...
2. `Main` 호출 모드 — 기존 `backfill` (월 전체 한 번에) ↔ Airflow `incremental` (`--run-date {{ds}}` 일별 + 전날 lookback)
...
**Q5. 그러면 Airflow가 기존을 대체하나?** — 층위를 나눠야 정확함.
...
- **하지만 통째로 교체하는 것은 아님**: Airflow는 새 변환 코드가 아니라 기존과 같은 `spark-submit`/python을 호출. 설계 결정 9대로 코어 정본 (Hive `activity` + `sql/wau.sql`)은 그대로 두고, Airflow는 선택적 확장으로 얹는 상위 자동화 레이어.
...
| `Main` 호출 | `backfill` (한 번에) | `incremental` (일별 catchup) |
...
"기존 vs Airflow 두 파이프라인"이 아니라, **같은 변환 코어를 (1) 손으로 backfill, (2) Airflow로 일별 자동 — 두 방식으로 돌리는 것**이며, `DailySplitter`는 (2)에서만 필요한 입력 랜딩 (Bronze) 추가.
Silver/Gold는 하나의 같은 테이블이라, Airflow는 "결과를 대체"하는 게 아니라 "그 결과를 만드는 실행을 자동화·통합"함.

---

참고: 코드 위치는 `src/main/scala/com/activitylog/Main.scala` (write 루프)·`PartitionWriter.scala` (staging → rename·`_SUCCESS`)·`DailySplitter.scala`.
라인 번호는 변경될 수 있으니 심볼로 찾을 것.
관련 문서 — 설계 스펙 §12, 런북 `docs/runbook/airflow.md`, README의 Airflow 절 [airflow/README.md](../../airflow/README.md)
