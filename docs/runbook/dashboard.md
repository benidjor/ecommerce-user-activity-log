...
근거: 설계 스펙 §3.4·§3.5·§11.
데이터를 변환하는 파이프라인 본체는 Scala로 작성하고, export·빌드를 맡는 이 레이어는 Spark를 쓰지 않으므로 Python으로 작성함 (프로젝트의 언어 사용 경계)
...
```
`marts.duckdb`는 서빙용 사본이라 저장소에 **커밋**함 (패턴3)
...
최초 1회 설정은 **순서가 중요**함 (repo 공개 → Pages 소스 지정 → 머지)
...
대시보드에서 자주 묻는 지표 정의와 데이터 경계 효과 정리.
모든 수치는 `marts.duckdb` 실측값.
