...
**검증 방식 (확정):** 기본은 **B = superpowers:verification-before-completion**(아래 Step 3에서 실제 쿼리 실행·실측). 시간 여유가 있으면 **A = `workflow` verify 추가**(두 WAU 수치를 다른 쿼리 표현·세션 경계로 적대적 교차검증). A는 `workflow`가 명시적 opt-in 필요(사용자가 "ultracode" 또는 "워크플로우로 WAU 교차검증" 요청 시 실행) → 사용 시 README "사용 도구"에 workflow 한 줄 추가. (전략 문서 §04: workflow verify는 선택, 없어도 충분)
