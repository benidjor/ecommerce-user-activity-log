...
object SparkSessionFactory {
  ...
  def create(appName: String): SparkSession =
    ...
    SparkSession.builder()
      ...
      // 파티션 overwrite 모드를 "dynamic"으로 설정:
      //   테이블 전체가 아니라 "처리 중인 파티션만" 덮어쓰기.
      //   멱등 일별 재실행 시 해당 날짜(event_date) 파티션만 교체 → 다른 날짜 안전.
      //   설계 결정 6의 보조 안전장치. 원자적 교체 자체는 PartitionWriter의 staging+rename이 담당.
